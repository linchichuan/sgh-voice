# 🎙 SGH Voice — 讓想法流動，不再卡在鍵盤上 (v1.6.0)
...
| 重點功能 | 說明 |
|------|------|
| **🌐 雲端雙引擎備援** | **(New)** 除了極速 Groq，現在新增 OpenRouter 支援 (200+ 模型)，確保你在靈感爆發時服務永遠在線。 |
| **📱 Android 封測招募** | **(New)** Android 版準備上線！我們正在招募首批測試夥伴，搶先體驗行動端的極速輸入。 |
| **🧠 7 規則 System Prompt** | **(New)** 全新設計的 LLM 後處理提示詞：填充詞清除、自我修正偵測、專有名詞校正、長文條列整理，一套 prompt 驅動所有引擎。 |
| **🎨 人性化文案優化** | 調整了內建的潤稿邏輯與提示詞，讓修正後的文字更具溫度，減少生硬的「AI 感」。 |
| **🌍 全球語系適配** | 自動偵測你的 macOS 語言 (日/中/英)，從選單到日誌，給你最親切的介面。 |
| **⚡ 毫秒級反應** | 延續不到一秒的處理神速，讓你的思考與輸入完全同步。 |
...
-   **5 引擎 LLM 路由**: Ollama (本地) / Groq / Claude / OpenAI / OpenRouter，自動 fallback。
-   **隱私第一**: 聲紋驗證在本地完成，只有你授權的雲端引擎會處理文字。
