...
- **2026-05-29**: v2.3.1 — Agent Team 程式碼審查與硬化（無 critical/high，12 議題修復）
  - **背景**：5 維度 agent team 審查（bug/security/繁中/效能/品質）+ 對抗式驗證，52 原始發現 → 確認 17 項真實問題。修復涵蓋核心 4 檔（`transcriber.py`/`app.py`/`dashboard.py`/`config.py`）。
  - **🔴 繁中防線補洞（最高優先規則）**：
    - 新增 `transcriber._apply_final_conversion(text)`（OpenCC s2twp，idempotent）。
    - 修復 Quick-Rewrite 改寫路徑繞過第三層繁中防護的破口：`app.py:_rewrite_text` 回傳前 + `dashboard.py:api_rewrite` 結果套用 OpenCC。
    - `_few_shot_pairs` 的 assistant 範例套 OpenCC，防止歷史殘留簡體被當範例強化。
  - **🔴 LLM 輸出清理統一**：新增 `transcriber._clean_thinking_tags(text)`（共用 `_THINK_PATTERN`），統一套用 5 引擎；補上原本漏清的 Claude/OpenAI/Ollama（reasoning 模型 `<think>` 標籤洩漏）。
  - **🟡 Ollama 退避修復**：`_local_llm_process` 成功時歸零 `_ollama_fail_count` + 清 `_ollama_backoff_until`，避免恢復後仍被舊計數懲罰成滿額退避。
  - **🟡 歷史落盤**：`app.py:_graceful_shutdown` 接上既有但從未呼叫的 `memory.flush_history()`，退出時補寫批量緩衝（最多 9 筆）。
  - **🟡 STT 去重重構**：STT 路由改用 `attempted` set，groq 每次轉寫最多呼叫 1 次（舊版長音訊情境可達 3 次，逾時疊加）。
  - **🟡 LLM 路由統一**：抽 `transcriber._build_llm_routes()` + `_run_llm_routes(pref, routes, mode)`，主流程與 retry 共用；retry 順帶補回原本缺的 API key 守衛與 `llm_attempt` 事件記錄。
  - **🟢 重複定義收斂**：
    - 風格提詞統一到 `config.STYLE_PROMPTS`（8 個共用鍵），`transcriber._STYLE_DIRECTIVES` / `app._REWRITE_STYLE_PROMPTS` / `dashboard.api_rewrite` 三處共用（dashboard 保留專屬 `meeting`）。
    - 用量統計抽 `transcriber._USAGE_FIELDS` 單一欄位清單 + `setdefault`，消除硬寫兩份鍵名。
  - **驗證**：全核心檔編譯通過；venv 實跑單元測試（think 標籤多型態、OpenCC 簡轉繁、Quick-Rewrite 繁中、STT 去重 call count）全綠。
  - **刻意未做**（風險/效益不對等）：`get_sys_lang` 去重（涉模組載入順序）、`local_llm` client 快取（本機 loopback 效益近零）、dashboard 錯誤訊息籠統化（僅綁 127.0.0.1 且傷除錯體驗）。
