...
- **2026-06-11**: v2.5.0 — 三 Agent 深度審查後的最佳化（Prompt 重構 + P0 功能修復）
  - **P0 功能修復**：
    - `recorder.py:_save()` 改每段唯一檔名（`voice_input_{ns}.wav`）— 修並行轉寫互相覆寫/音訊遺失的 race
    - `transcriber.py:_validate_llm_result()` edit 模式豁免 overlap/長度檢查（翻譯/Email 輸出先前必被誤殺退回原文），只擋 `_EDIT_REPLY_MARKERS` 前言/拒絕
    - `_CONV_MARKERS` 改「原文開頭豁免」— 口述「好的，沒問題…」不再被當幻覺丟棄
    - LLM/STT client `max_retries=0` + timeout 改 `with_options()` per-request 覆寫 — fallback 每家從 ~16s 降回 5s，STT 動態 timeout 不再被 cache 鎖死
    - STT fallback 加 `_stt_attempted` 去重（Groq 異常時不再連試 3 次）
    - Dashboard port fallback 時動態加入 `_ALLOWED_ORIGINS`（7866+ 不再被自家 CSRF 403）
    - cancel flag 在 inflight 歸零時兜底清理（不再誤吞下一段正常口述）
  - **Prompt 重構**：
    - `_DICTATE_SYSTEM` 改版：台灣用語對照防護（视频/软件 等繁體字形大陸詞 OpenCC 擋不住）、解 ±20% 長度與去填充詞的矛盾、few-shot 範例宣告、injection 英文例句
    - `_EDIT_SYSTEM` → `config.EDIT_SYSTEM_PROMPT`（`<command>/<text>` 結構化防 injection + 繁中規則）
    - 三份 rewrite 風格字典統一收斂到 `config.REWRITE_STYLE_DIRECTIVES`（transcriber/app/dashboard 同源）
    - Quick-Rewrite 與 Dashboard rewrite 補 OpenCC 終層（translate_ja 豁免）
    - `memory.get_style_profile()` 拔掉「商務語氣」預設值（與逐字轉寫契約衝突，是改寫型幻覺誘因）
    - Whisper `_LANG_HINT` 去指令化（initial_prompt 是 biasing 不是指令通道），改繁中敘述句
    - App styles（notes/search/ai_chat）與 medical scene extra 對齊逐字契約
    - `medical_consultation` SOAP 場景重構為 `edit_directive` 機制（transcriber 偵測後自動切 edit 模式，舊版 SOAP 輸出必被幻覺偵測誤殺）
  - **P2**：
    - corrections ASCII 規則加 word boundary（Cloudflare 不再變 Claudeflare）
    - `save_dictionary` 原子寫入；smart_replace 寫入驗證 + 套用端防禦
    - 連續模式四連修：強制切片不誤裁 1.3s 語音、stop 最後一段不再被丟、pre-roll 300ms 保住句首弱輔音、stream 死亡 `on_stopped` 自動復原；paste 補 `_paste_lock`；`transcribe(history_mode=)` 消除重複 history
  - **測試**：`tests/test_v250_optimizations.py` 新增 14 個回歸測試，全套 71 passed

- **2026-04-29**: v2.1.0 — Personalization & Productivity Release
  - **A. 個人化 Few-shot 後處理**：每次 LLM 後處理會自動把最近 3 筆 `whisper_raw → final_text` 範例以 user/assistant pair 注入 messages，讓模型沿用使用者的標點/用詞習慣。所有 5 個引擎（Ollama/Groq/Claude/OpenAI/OpenRouter）統一支援；`mode="edit"`（rewrite API）跳過。
    - `memory.py`: 新增 `get_few_shot_examples(n=3)`，優先 `edited=True`，fallback 到 raw≠final 隱性正例。
    - `transcriber.py`: 新增 `_few_shot_pairs(mode)` helper；5 個 LLM 函數的 messages 改為 `[system, *few_shot, user]`（Claude 走 system 參數，messages 只放 few_shot+user）。
    - `config.py`: 新增 `enable_fewshot=True`、`fewshot_count=3`。Dashboard 設定頁可開關與調整筆數。
  - **A2. Dictionary 從歷史學習工具**：
    - `scripts/dictionary_promote_from_history.py`：CLI，token 級 diff 統計 + 多重守門（`_is_meaningful_correction` + 子字串幻覺過濾 + OpenCC 已處理過濾），預設 `--min-freq 5`、`--source both`。
    - `POST /api/dictionary/promote_from_history`：dry-run / apply 兩段式 endpoint。
    - Dashboard 修正規則卡片新增「🎯 從歷史學習」按鈕（搭配既有「🧹 清理壞規則」）。
  - **B. 全域 Quick-Rewrite 熱鍵**：選取任意 App 文字 → 按熱鍵 → LLM 改寫 → 自動貼回。
    - `app.py`: VoiceEngine 新增 `rewrite_selection()`、`_simulate_cmd_c()`、`_rewrite_text()`；新函數 `setup_rewrite_hotkey()`（獨立 NSEvent 全域監聽，不影響原錄音熱鍵）。沿用 `paste_text()` 的剪貼簿保護 + LLM fallback 鏈（Groq → OpenRouter → Claude → OpenAI）。
    - `config.py`: 新增 `rewrite_hotkey="right_option+r"`、`default_rewrite_style="concise"`（concise/formal/casual/email/technical/translate_en/translate_ja/translate_zh）。
    - Dashboard 設定頁新增「✏️ Quick-Rewrite」區塊。
  - **C. 連續錄音模式 + VAD 自動分段**：
    - `recorder.py`: 新增 `start_continuous(on_segment, on_voice_change)`，含 voice/silence 邊界偵測、最短/最長片段保護、尾端靜音裁切。
    - `app.py`: VoiceEngine 新增 `start_continuous_mode()`、`stop_continuous_mode()`、`toggle_continuous_mode()`；新函數 `setup_continuous_hotkey()`。每段切片自動寫入 history（mode="continuous"）並走 `paste_text()`。
    - `config.py`: 新增 `continuous_hotkey=""`（空=關閉）、`continuous_silence_duration=1.5`、`continuous_min_segment_duration=0.6`、`continuous_max_segment_duration=30.0`。
    - Dashboard 設定頁新增「🎙 連續錄音模式（實驗性）」區塊。
  - **錄音檔搬離隱藏目錄**：`backup_audio_dir` 從 `~/.voice-input/audio_backup` 改為 `/Volumes/Satechi_SSD/voice-input/audio_backup`（285 個歷史檔已搬移）。SSD 未掛載時 `app.py:424` 自動 skip backup。
...
- **2026-06-13**: 修復「錄音中…之後毫無下文」災情（PortAudio 裝置清單過期）
  - 症狀：app 長跑數日 + 凌晨切換音訊裝置後，每段錄音 InputStream 開啟失敗 → 收到 0 chunk → `stop_and_process()` 靜默回 idle，使用者只看到 🔴錄音中…
  - `recorder.py`：新增 `_open_input_stream()`（開失敗 → `sd._terminate()+sd._initialize()` 刷新後重試一次，自癒）、`_reinit_portaudio()`、`last_error` 追蹤、`start(on_error=)` 回呼；stream 半路死亡也刷新 PortAudio 讓下一段能重開（含 continuous loop）
  - `app.py`：新增 `_on_recorder_error()`（stream 開失敗立刻重置 engine 狀態 + overlay idle + 顯示錯誤 + hotkey reset）；`stop_and_process()` 空音訊路徑不再靜默，會帶出 `recorder.last_error`
  - 測試：`tests/test_recorder_stream_recovery.py` 5 個回歸測試，全套 76 passed
